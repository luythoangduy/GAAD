...
# ===========================================================================
#  GraphAnomalyAdapter  —  replaces ViTill / knowledge distillation pipeline
# ===========================================================================

class GraphAnomalyAdapter(nn.Module):
    """
    Graph-Anchored Anomaly Detection model.

    Architecture
    ------------
    1. Frozen DINO encoder  →  extract patch tokens from `target_layers`
    2. Layer-mean fusion    →  [B, N_patches+special, embed_dim]
    3. Adapter blocks       →  lightweight Transformer layers (learned)
    4. Projection head      →  map to CLIP anchor dimension D_anchor
    5. At inference: cosine distance to learnable edge anchors → image score

    Parameters
    ----------
    encoder        : DINO ViT (frozen)
    adapter        : nn.ModuleList of Transformer Block (from vision_transformer.py)
    proj           : nn.Linear(embed_dim → D_anchor)
    target_layers  : which encoder layers to extract & mean-fuse
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Returns adapter_out: [B, N_patches, D_anchor]
        (class & register tokens are stripped before returning)
        """
        # ---- 1. Multi-layer feature extraction (frozen DINO) ----
        ...
        # ---- 2. Mean-fuse across selected layers ----
        # Each element: [B, 1+num_reg+N, D]
        fused = torch.stack(layer_feats, dim=1).mean(dim=1)  # [B, 1+r+N, D]

        # ---- 3. Adapter blocks (trainable) ----
        x_adapt = fused
        for blk in self.adapter:
            x_adapt = blk(x_adapt)      # [B, 1+r+N, D]

        # ---- 4. Strip special tokens, project ----
        n_special = 1 + self.encoder.num_register_tokens   # CLS + registers
        patch_tokens = x_adapt[:, n_special:, :]           # [B, N, D]
        out = self.proj(patch_tokens)                       # [B, N, D_anchor]

        return out   # [B, N_patches, D_anchor]
